...
$(function () {
    ...
    function createExportDialog() {
        ...
        exportTypes = ['zip_minimal', 'zip_with_Csound_libs'];
        ...
        /*====================================
        RW 2013 Choose csd file to use in project.
        ======================================*/
        $('<button />').attr('id', 'export-csdFile')
                .text("Select Csound file")
                .addClass("buttonStyle")
                .appendTo(exportMenu);
    ...
    function createZipAndExport(pid, ribFile, type) {
        ...
        if(type!="zip_minimal"){
            files.push('pnacl/Release/csound.nmf');
            files.push('pnacl/Release/csound_unstripped.pexe');
            files.push('pnacl/pnacl_csound.html');
            files.push('csound.js');
        }
        ...
        /*========================================================================
        RW 2013
        Add Csound file to export package and parse html file for input types and 
        add javsascript functions. Currently only works for sliders, but can be extended 
        ...
        jsModuleDidLoad =   '\nfunction moduleDidLoad() {\n'+
                            'csound.PlayCsd("http/'+$.rib.pmUtils.CsdFileName+'");\n'+
                            '}\n\n';
        ...
        jsCsoundEngine =    '<!--pNaCl csound module-->\n' +
                            '<div id="engine"></div>\n';

        //we only need ot add this is there is a textarea called console
        if(completeHTMLText.search('<textarea id="console">')>0)
        jsHandleMessages =  '\nvar count = 0;\n'+
        ...
        //console.log(complteJavascript);  


        completeHTMLText = completeHTMLText.replace("</body>", jsCsoundEngine+complteJavascript+'\n</body>');

        //have to replace ids in <div data-role="fieldcontain"> fields as they mess up
        //event handlers. Ugly hack. Better approach would be to modify the Slider type in widgets.js
        console.log(nodeIds.length);
        for( var i=0;i<nodeIds.length;i++){
            var searchString = '<div data-role="fieldcontain" id="'+nodeIds[i]+'">';
            console.log(searchString);
            var replaceString = '<div data-role="fieldcontain" id="'+nodeIds[i]+'_dr">';
            console.log(replaceString);
            completeHTMLText = completeHTMLText.replace(searchString, replaceString);

        }

        //resultHTML && zip.add("index.html", newText);
        zip.add($.rib.pmUtils.CsdFileName, $.rib.pmUtils.CsdFileContents);
        //========================================================================

        resultHTML && zip.add("index.html", completeHTMLText);
    ...
    RW 2013 Choose csd file to use in project.
    ======================================*/
    function launchFileOpenDialog(){
        console.log("lauching file open");
            $.rib.fsUtils.upload("csound", $.rib.pmUtils.projectDialog, function(file) {
                console.log("test");
                var reader = new FileReader();
                reader.onload = function(event) {
                    var contents = event.target.result;
                    //console.log("File contents: " + contents);
                    $.rib.pmUtils.CsdFileName = file.name;
                    $.rib.pmUtils.CsdFileContents = contents;
                };
    ...
    function exportPackage(ribFile) {
        ...
        exportDialog.find("button#export-csdFile").click(function () {
              launchFileOpenDialog();
        });

        exportDialog.find("button#export-zip_minimal").click(function () {
            if($.rib.pmUtils.CsdFileName.length==0){
                alert("You need to select a .csd file");
            }
            else{
            createZipAndExport(pid, ribFile, 'zip_minimal');
            exportDialog.dialog('close');
            }
        });

        exportDialog.find("button#export-zip_with_Csound_libs").click(function () {
            if($.rib.pmUtils.CsdFileName.length==0){
                alert("You need to select a .csd file");
            }
            else{
            createZipAndExport(pid, ribFile, 'zip_with_Csound_libs');
            exportDialog.dialog('close');
            }
        });
